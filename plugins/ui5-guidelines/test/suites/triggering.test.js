...
function matchSkill(prompt, skills) {
  const scores = skills.map(skill => {
    ...
    const overlap = descWords.filter(w => w.length > 3 && promptWords.includes(w)).length;
    score += overlap * 0.3;
...
module.exports = function runTriggeringTests(framework) {
  ...
  // Load test cases
  const testCasesPath = path.join(framework.pluginRoot, 'test', 'fixtures', 'trigger-cases.json');
  let testCases;

  try {
    testCases = require(testCasesPath);
  } catch (err) {
    console.log('  ⚠️  No trigger test cases found at fixtures/trigger-cases.json');
  ...
  testCases.tests.forEach(testCase => {
    framework.test(`"${testCase.prompt.substring(0, 50)}..."`, () => {
      ...
      if (testCase.should_trigger) {
        ...
        // Should NOT trigger any skill
        if (matched === testCase.expected_skill) {
          throw new Error(
            `Should not trigger "${testCase.expected_skill}", but it did`
          );
        }
      }
    });
  ...
  if (accuracy < 80) {
    console.log('  ⚠️  Accuracy below 80% - consider improving skill descriptions');
  }
};
